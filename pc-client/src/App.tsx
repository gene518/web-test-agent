            ...
            {messages.length === 0 && !interrupt ? (
              <div className="empty-state">
                ...
                <div className="prompt-examples">
                  <button onClick={() => setInput("为登录页面生成一份完整的测试计划")}>生成登录页测试计划</button>
                  <button onClick={() => setInput("读取已有测试计划并生成 Playwright 脚本")}>生成 Playwright 脚本</button>
                  <button onClick={() => setInput("运行现有测试并修复失败用例")}>调试失败测试</button>
                </div>
              </div>
            ) : (
              <>
      ...
      {logOpen && (
        <div className="modal-backdrop" role="presentation" onMouseDown={() => setLogOpen(false)}>
          <section className="modal log-modal" role="dialog" aria-modal="true" aria-labelledby="log-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <pre className="log-content">{backendLog}</pre>
          </section>
        </div>
      )}
    </main>
