...
class GeneratorAgent(BaseSpecialistAgent):
    ...
    def _build_runtime_context_prompt(self, *, state: WorkflowState, workspace_dir: Path | None) -> str:
        ...
        prompt_sections = [
            ...
            f"- resolved_test_plan_files: {self._format_prompt_value(display_resolved_plan_files)}",
            ...
            f"- 只有当 `expected_test_scripts` 中列出的 {len(expected_test_scripts)} 个脚本都已写入目标路径时，本阶段才算完成；优先使用 `generator_write_test`，也允许使用已开放的文件写入工具。",
