...
def validate_project_name(project_name: str) -> str:
    ...
    if (
        ...
        raise RuntimeError(
            "自动化工程名字只能包含单个路径段，不能使用绝对路径、目录分隔符或 `..`。"
        )
    return normalized_name


def resolve_path_within_root(*, root: Path, relative_path: Path, label: str) -> Path:
    """解析相对路径并确保结果仍位于指定根目录中。"""
    ...
    try:
        ...
        raise RuntimeError(
            f"{label}不能逃逸自动化工程根目录：`{relative_path}`。"
        ) from exc
...
def resolve_autotest_project_dir(
    *,
    ...
    demo_project_dir = ensure_demo_project(
        automation_root=automation_root, bundled_template_dir=bundled_template_dir
    )
    ...
    if raw_project_dir:
        ...
        if not workspace_dir.is_absolute():
            ...
            workspace_dir = resolve_path_within_root(
                root=automation_root,
                relative_path=workspace_dir,
                label="相对 `project_dir` ",
            )
    else:
        if not project_name:
        ...
        workspace_dir = resolve_path_within_root(
            root=automation_root,
            relative_path=Path(validate_project_name(project_name)),
            label="`project_name` ",
        )
    ...
    if workspace_dir.exists():
        if not workspace_dir.is_dir():
            raise RuntimeError(
                f"自动化工程路径 `{workspace_dir}` 已存在但不是目录，无法继续。"
            )
...
def ensure_demo_project(*, automation_root: Path, bundled_template_dir: Path) -> Path:
    ...
    if demo_project_dir.exists():
        if not demo_project_dir.is_dir():
            raise RuntimeError(
                f"默认 demo 工程路径 `{demo_project_dir}` 已存在但不是目录，无法继续。"
            )
...
def copy_template_project(source_dir: Path, target_dir: Path) -> None:
    ...
    shutil.copytree(
        source_dir,
        target_dir,
        ignore=shutil.ignore_patterns(*AUTOTEST_TEMPLATE_IGNORE_PATTERNS),
    )
