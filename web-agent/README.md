...
| 分组 | 变量 | 用途 |
| --- | --- | --- |
| 兼容配置 | `OPENAI_API_KEY` / `OPENAI_BASE_URL` | OpenAI 兼容模型服务的凭证和基础地址 |
| 兼容配置 | `MASTER_MODEL` / `SPECIALIST_MODEL` | Master 和 Specialist 的默认模型 |
| 角色配置 | `MASTER_LLM__*` / `SPECIALIST_LLM__*` | 按角色配置 family、channel、model、API Key、Base URL、thinking 等；配置后优先于扁平字段 |
...
默认地址为 `http://127.0.0.1:2024`，可在启动脚本进程中通过 `BACKEND_PORT` 覆盖。后端日志写入仓库根目录的 `start/backend.log`。
...
2. `web-agent-scheduler` 常驻进程读取 JSON，按 Cron 执行 Playwright，再进入总结阶段。

第一条链路完成后会输出固定以 `**Scheduler 阶段**` 开头的配置阶段总结。成功和失败都包含状态与配置文件；成功时还包含项目目录、新建/更新操作、任务 ID、Cron、有头/无头模式、启用状态、测试范围和 Scheduler 日志。这些稳定字段由代码生成，模型补充内容只作为“说明”追加。桌面客户端可对其中的配置文件、项目目录、测试范围和日志路径进行安全的文件管理器定位。
