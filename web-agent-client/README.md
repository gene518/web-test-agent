...
- 新建对话和查看服务端历史对话
- 不同对话使用独立流控制器并发执行；切换对话不会取消后台任务，也不会把运行状态串到当前对话
- 使用模型总结标题，并在空闲时限速回填当前已加载的旧历史标题
- 展示 Agent 消息、工具调用参数和工具结果
- 长对话按轮次渐进渲染，保留每个对话的阅读位置，并提供“回到最新消息”按钮
- 重新加入运行中的线程流
...
每个已选中或仍在运行的对话都有独立的 LangGraph 流实例。一个对话在后台执行时，可以切换到另一个历史对话继续提交；客户端分别保存消息、排队/运行/补参/取消状态、错误提示和输入草稿。同一对话仍拒绝重复运行，不同对话由后端 worker 并发执行。

新对话标题复用 Master 首轮意图分类模型生成，并随 checkpoint 持久化。历史列表只提取标题和首条消息，不批量下载完整 transcript；缺少模型标题的当前已加载历史项会在没有前台任务时单并发回填，失败不阻塞对话。

消息时间线按用户轮次组织，首次渲染最后 20 轮，上滑到顶部后每次追加 20 轮并保持可见锚点。用户主动离开底部后，流式输出不会强制拉回；浮动向下按钮可返回最新消息。阅读位置和输入草稿都按对话隔离，应用重启后重新默认定位最新。
...
1. 检查目标端口的 `/info`。
2. 先停止本客户端此前启动并验证归属的后端；若端口由当前仓库 `web-agent` 启动的 LangGraph 服务占用，客户端可在“重新启动”时安全接管；其他外部服务（包括其他仓库的 LangGraph 实例）仍会报告冲突并拒绝终止。
...
客户端仅在识别到 `**Plan 阶段**`、`**Generator 阶段**`、`**Healer 阶段**` 或 `**Scheduler 阶段**` 这类规范总结时，把反引号中像文件或目录的值显示为带类型图标的紧凑可点击引用。界面显示 basename，完整路径保留在 tooltip 和无障碍标签中。
...
- 无状态旧标题回填图 ID 固定为 `web-autotest-thread-title`。
