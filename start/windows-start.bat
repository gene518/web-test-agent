@echo off
REM =====================================================================
REM windows-start.bat - Single-file Windows entry for Web AutoTest Agent.
REM ---------------------------------------------------------------------
REM Usage:
REM   - Double-click this file: runs with default mode "start".
REM   - Command line: windows-start.bat [start^|end^|logs]
REM
REM File layout:
REM   - The cmd section below is parsed by cmd.exe; it invokes powershell
REM     to read THIS file and execute the PowerShell body that lives after
REM     the `:POWERSHELL_SECTION` marker line.
REM   - This is a classic polyglot trick so the user only sees one .bat,
REM     while the real implementation stays in PowerShell (needed for port
REM     detection, process tree management, version parsing, etc).
REM   - Keep the cmd section ASCII-only; cmd.exe on Chinese Windows reads
REM     files as GBK by default, so any non-ASCII byte here produces the
REM     classic "not recognized as an internal or external command" error.
REM   - The PowerShell section is read back via UTF-8, so Chinese text
REM     there is fine.
REM =====================================================================
...
REM Spawn powershell.exe, ask it to re-read THIS file, skip everything up
REM to and including the :POWERSHELL_SECTION marker, and execute the rest.
REM We use Invoke-Expression instead of a new ScriptBlock so that the
REM $script:* scope modifiers used below resolve to the current runspace.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$markerIndex=-1;" ^
  "for($i=0;$i -lt $allLines.Count;$i++){ if($allLines[$i] -match '^:POWERSHELL_SECTION\s*$'){ $markerIndex=$i; break } };" ^
  "if($markerIndex -lt 0){ Write-Host '[windows-start.bat] POWERSHELL_SECTION marker not found. File is corrupted.'; exit 2 };" ^
  "$psBody=($allLines[($markerIndex+1)..($allLines.Count-1)] -join [Environment]::NewLine);" ^
  "Invoke-Expression $psBody"

set "PS_EXIT=%ERRORLEVEL%"

echo.
echo [windows-start.bat] Script finished. Exit code: %PS_EXIT%
...
:POWERSHELL_SECTION
# 本段由 bat 头部的 Invoke-Expression 执行；$Mode 与 $ScriptPath 已经在外层 runspace 设好。

$ErrorActionPreference = "Stop"

# ScriptPath 指向 .bat 自身；ScriptDir 用它所在目录。
...
switch ($Mode.ToLowerInvariant()) {
  ...
  "logs" { Show-Logs }
  default {
    Write-Host "用法：windows-start.bat [start|end|logs]"
    exit 1
  }
}
