...
function EditActionCard({
  ...
  const buttonText =
    editResponse.acceptAllowed && !editResponse.editsMade ? "批准" : "提交";
...
export function InboxItemInput({
  ...
  const onEditChange = (
    ...
    setHumanResponse((prev) => {
      if (response.type !== "edit" || !response.edited_action) {
        console.error("编辑回复类型不匹配", response.type);
        return prev;
      }
  ...
  return (
    <div className="flex w-full max-w-full flex-col items-start justify-start gap-2">
      ...
      <div className="flex w-full flex-col items-stretch gap-2">
        ...
        {isLoading && <p className="text-sm text-gray-600">正在提交决策...</p>}
        {selectedSubmitType && supportsMultipleMethods && (
          <p className="text-xs text-gray-500">
            当前选择：{prettifyText(selectedSubmitType)}
          </p>
        )}
      </div>
    </div>
